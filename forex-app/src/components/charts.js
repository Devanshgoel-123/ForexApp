...
function ChartCompt({currencyL,currencyR}) {
 ...
 const [valueArrayRatio,setvalueArrayRatio]=useState([]);
...
const [gotDate,setgotDate]=useState(true);
  ...
  async function fetchHistoryData() {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      try {
        const response = await axios.get(`http://data.fixer.io/api/${formattedDate}?access_key=f0cdd543eb273ebd80028d00e9d34842`);
        if (response && response.data) {
          const ratesData = response.data.rates;
          historicalData.push(ratesData);
          let currencyRatio=((ratesData[currencyL])/(ratesData[currencyR]));
          ratioData.push(currencyRatio);
  ...
  useEffect(() => {
    if (rates.length > 0) {
      ...
      const data = {
        labels: dateArr,
        datasets: [{
          label: `${currencyL} VS ${currencyR} `,
          data: valueArrayRatio,
          backgroundColor: [
            'rgba(255, 26, 104, 0.2)'
          ],
          borderColor: [
      ...
      const config = {
        ...
        options: {
          scales: {
            y: {
             max:valueArrayRatio[0]+0.5,
             min:valueArrayRatio[3]-0.5
            }
          }
        }
      };
...
<div className="chartCard">
  ...
  <canvas className="trendChart" id="Chart0"></canvas>
